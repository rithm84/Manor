...
import { formatNoteTime, pagesForScope, scopeTitle, treeRows } from './notes/notesModel'
import type { NotesScope } from './notes/notesModel'
import { createNoteSaveQueue, flushPendingNote } from './notes/notesAutosave'
...
export function NotesPage(): ReactNode {
  ...
  const flushPendingOnTeardown = useCallback((): void => {
    if (timerRef.current !== null) {
      window.clearTimeout(timerRef.current)
      timerRef.current = null
    }
    ...
    flushPendingNote(window.manor.notes, pending)
  ...
  const openPage = useCallback(async (pageId: string): Promise<void> => {
    ...
    setMenuOpen(false)
    try {
      const touched = await window.manor.notes.touchPage(pageId)
      setData((current) => ({
        ...current,
  ...
  useEffect(() => {
    let active = true
    window.manor.notes.load(notesSeed)
      .then((state) => {
        if (!active) return
        setData(state)
    ...
    return (): void => {
      ...
      if (timerRef.current !== null) window.clearTimeout(timerRef.current)
      try {
        flushPendingOnTeardown()
      } catch (flushError) {
        window.reportError(flushError)
      }
    }
  }, [flushPendingOnTeardown])
  ...
  useEffect(() => {
    const onBeforeUnload = (event: BeforeUnloadEvent): void => {
      try {
        ...
        setSaveState('error')
        setError(flushError instanceof Error ? flushError.message : String(flushError))
  ...
  const createPage = useCallback(async (parentPageId: string | null): Promise<void> => {
    ...
    try {
      ...
      const state = await window.manor.notes.createPage({
        title: 'Untitled',
  ...
  const importMarkdown = async (event: ChangeEvent<HTMLInputElement>): Promise<void> => {
    ...
    try {
      ...
      const previousIds = new Set(data.pages.map((page) => page.id))
      const state = await replaceState(window.manor.notes.createPage({
        title: file.name.replace(/\.(md|markdown)$/i, '') || 'Imported note',
        folderId: activeFolderForScope(scope),
        parentPageId: null,
        contentJson
      }))
  ...
  return (
    <PageShell title="Notes" fullBleed={true}>
      <div className="notes-workspace">
        <nav className="notes-nav" aria-label="Notes navigation">
          ...
          <div className="notes-nav-foot">
            ...
            <input ref={importRef} type="file" accept=".md,.markdown,text/markdown" onChange={(event) => void importMarkdown(event)} hidden />
          </div>
        </nav>
        ...
        <section className="notes-list" aria-label="Note list">
          ...
          <div className="notes-list-scroll">
            ...
            {rows.map(({ page, depth }, index) => (
              <button type="button" key={page.id} data-note-index={index} className={`notes-list-row${selectedId === page.id ? ' is-selected' : ''}`} style={{ paddingLeft: `${12 + Math.min(depth, 4) * 14}px` }} onClick={() => void openPage(page.id)} onKeyDown={(event) => moveWithKeyboard(event, index)}>
      ...
      <FolderDialog value={dialog?.kind === 'folder' ? dialog : null} onClose={() => setDialog(null)} onDelete={async (folderItem) => {
        await replaceState(window.manor.notes.deleteFolder(folderItem.id))
        if (scope === `folder:${folderItem.id}`) setScope('all')
        setDialog(null)
      }} onSubmit={async (name, folderItem) => {
        if (folderItem === null) {
          const state = await replaceState(window.manor.notes.createFolder({ name, parentFolderId: null }))
          if (state === null) return
        ...
        } else await replaceState(window.manor.notes.renameFolder({ id: folderItem.id, name }))
        setDialog(null)
      }} />
      <MoveDialog value={dialog?.kind === 'move' ? dialog.page : null} folders={data.folders} pages={data.pages} onClose={() => setDialog(null)} onSubmit={async (page, folderId, parentPageId) => {
        await replaceState(window.manor.notes.movePage({ id: page.id, folderId, parentPageId }))
        setDialog(null)
      }} />
      <DeleteDialog page={dialog?.kind === 'delete' ? dialog.page : null} onClose={() => setDialog(null)} onConfirm={async (page) => {
        await mutateSelected(() => window.manor.notes.permanentlyDeletePage(page.id))
        setDialog(null)
      }} />
    </PageShell>
  )
