...
/** 12 polls at 5s covers about a minute of normalization, then polling stops. */
...
export interface CapturesController {
  ...
  /** Re-runs normalization for a failed entry and restarts the pending poll. */
...
export function useCaptures(): CapturesController {
  ...
  const load = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const entries = await window.manor.kb.list()
      if (!aliveRef.current) return
  ...
  const removeCapture = useCallback(async (entryId: string): Promise<void> => {
    await window.manor.kb.remove(entryId)
    if (!aliveRef.current) return
    setState((previous) =>
      previous.kind === 'ready'
  ...
  const retryCapture = useCallback(async (entryId: string): Promise<void> => {
    ...
    try {
      await window.manor.kb.normalize(entryId)
    } catch (error) {
      if (!aliveRef.current) return
