...
import { habitSteps, markForDate, targetUnitCount } from '../../../shared/habits'
    ...
    <div className="habit-detail">
      <div className="habit-detail-identity">
        ...
        <span className="habit-detail-kind">
          {definition.kind === 'binary'
            ? 'One tap'
            : targetUnitCount(definition.targetLabel) !== null &&
                habitSteps(definition.targetLabel).length === targetUnitCount(definition.targetLabel)
              ? `One at a time to ${definition.targetLabel}`
              : `In quarters to ${definition.targetLabel}`}
      ...
      <details className="habit-rules">
        ...
        <div className="habit-rules-body">
          <p>Miss a day and you can spend a freeze on this habit the next day to keep its streak.</p>
      ...
      {status === 'retired' ? (
        <section className="habit-retire-zone">
          <div>
            <span className="habit-retire-title">Archived</span>
            <span className="habit-retire-copy">Bring it back to track it again from today.</span>
          </div>
          <Button variant="subtle" icon={<Play size={14} />} onClick={onResume}>
            Reactivate
          </Button>
        </section>
      ) : (
        <section className="habit-retire-zone">
          <div>
            ...
            <span className="habit-retire-copy">Moves it to your archive. History stays.</span>
          </div>
