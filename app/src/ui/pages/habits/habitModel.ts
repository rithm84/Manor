...
export interface HabitViewModel {
  ...
  /** Whether a freeze can still be spent on the selected day. */
  canFreeze: boolean
  frozen: boolean
}
...
export function habitViewModel(
  ...
  const mark = markForDate(state, habit, selectedDate)
  const pool = state.pools.find((candidate) => candidate.month === monthKey(selectedDate)) ?? null
  return {
    definition: habit,
    status,
    selectedStatus,
    ...
    canFreeze:
      (mark === 'missed' || mark === 'partial') &&
      selectedDate === addDays(state.today, -1) &&
      (pool?.balance ?? 0) > 0,
    frozen: mark === 'frozen'
  }
}
...
export function stepAmountLabel(targetLabel: string, value: number): string {
  // The count can sit anywhere in the label ("Take 2 tablets"); the unit is
  // whatever follows it.
  const match = /(\d+(?:\.\d+)?)\s*(.*)$/.exec(targetLabel.trim())
  if (match === null) {
    return `${value}%`
  }
  ...
  // "1 tablets" reads wrong in the screen-reader label; drop the plural s
  // for a single unit when the suffix is a simple plural word.
  const unit = amount === 1 && /^[A-Za-z]+s$/.test(suffix) ? suffix.slice(0, -1) : suffix
  return `${amount}${unit === '' ? '' : ` ${unit}`} / ${targetLabel}`
}
...
export function monthSummary(state: HabitsState, month: string): HabitMonthSummary {
  ...
  /* A retired habit keeps its history, so it stays in the months it ran for;
     markForDate reads 'inactive' from the retirement date onward. */
