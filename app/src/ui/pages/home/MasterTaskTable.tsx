import { Bookmark, CheckCircle2, Filter, Layers, Plus, RotateCcw, Search, Trash2, X } from 'lucide-react'
...
// Done is not a filter: completed tasks live behind the Completed view.
...
export function MasterTaskTable({ tasks, contexts, today, savedViews, onOpenTask, onQuickActions, onSaveView, onDeleteView }: MasterTaskTableProps): ReactNode {
  ...
  const visible = useMemo(() => {
    ...
    /* Done tasks live behind the Completed view: the working table never
       mixes them in, and the Completed view shows nothing else. */
  ...
  return (
    <section className="master" aria-label="Master task view">
      <div className="master-toolbar">
        ...
        <button
          type="button"
          className={`master-tool-button${completedView ? ' is-active' : ''}`}
          aria-pressed={completedView}
          onClick={() => setCompletedView((showing) => !showing)}
        >
