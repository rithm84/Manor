...
/** Title for an imported file: extension off, and the 32-hex page id Notion
    appends to exported filenames trimmed away. */
export function importedNoteTitle(fileName: string): string {
  const base = fileName.replace(/\.(md|markdown)$/i, '').replace(/\s+[0-9a-f]{32}$/i, '').trim()
