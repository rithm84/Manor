import { History, Settings2, Timer } from 'lucide-react'
...
import { Tooltip, ViewTabs } from '../components/ui'
...
type PomodoroView = 'timer' | 'history'

const VIEWS: readonly PomodoroView[] = ['timer', 'history']
...
export function PomodoroPage(): ReactNode {
  ...
  const [view, setView] = useDevicePreference<PomodoroView>('pomodoro.view', VIEWS, 'timer')
  ...
  return (
    <div className={`pomo is-${view}`}>
      <header className="pomo-header">
        ...
        <div className="pomo-header-actions">
          <ViewTabs
            ...
            onChange={setView}
            tabs={[
              { value: 'timer', label: 'Timer', icon: <Timer size={14} /> },
      ...
      {view === 'timer' ? (
        ...
        <div className="pomo-history" data-testid="pomodoro-history">
          <MonthStats
            stats={stats}
            today={today}
