...
/**
 * Streak from real attempt history: consecutive days with at least one
 * attempt, ending today or yesterday (today's attempt not yet required).
 * Legacy summaries (the signed-out demo) carry their own streak instead.
 */
export function attemptStreak(attempts: readonly LeetCodeAttempt[], today: string): number {
  const days = new Set(attempts.map((attempt) => attempt.date))
  let start = today
  if (!days.has(start)) start = previousIsoDate(start)
  let streak = 0
  for (let date = start; days.has(date); date = previousIsoDate(date)) {
    streak += 1
  }
  return streak
}
