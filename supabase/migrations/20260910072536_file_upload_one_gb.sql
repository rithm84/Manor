-- Decimal 1 GB attachment policy; staging global Storage cap is configured separately.
alter table public.file_objects drop constraint file_objects_size_check;
alter table public.file_objects add constraint file_objects_size_check check(size between 1 and 1000000000);
update storage.buckets set file_size_limit=1000000000 where id='manor-files';
...
begin
 if op='allocate_file' then
  ...
  if (p->>'size')::bigint>1000000000 then raise exception using errcode='PT413',message='Choose a file no larger than 1 GB.',detail='file_too_large'; end if;
