-- The desktop app's webview runs at the literal origin tauri://localhost, which the integration OAuth
-- state must be allowed to record as its return origin; integration-callback turns that origin into the
-- desktop app's URL scheme. Everything else in manor_integration_store is unchanged.
...
begin
 ...
 when 'oauth_start' then
  ...
  if p_input->>'returnOrigin' is null or (p_input->>'returnOrigin' !~ '^https?://[^/?#]+$' and p_input->>'returnOrigin' <> 'tauri://localhost') then raise exception 'A validated application return origin is required'; end if;
...
-- The column constraint from web_integration_return_origin accepts the same desktop origin.
alter table manor_private.integration_oauth_states drop constraint integration_oauth_states_return_origin_check;
alter table manor_private.integration_oauth_states add constraint integration_oauth_states_return_origin_check
  check (return_origin is null or return_origin ~ '^https?://[^/?#]+$' or return_origin = 'tauri://localhost');
