...
import { assertAllowedOrigin, settingsReturnUrl } from '../_shared/origins.ts'

// Return only the short-lived code to its original browser. Token exchange requires
// the original authenticated owner plus that browser's PKCE verifier. A connection started
// in the desktop app returns as a deep link on that build's URL scheme.
Deno.serve(async(request:Request):Promise<Response>=>{
 const url=new URL(request.url)
 let target=settingsReturnUrl(required('MANOR_ORIGIN'))
 try {
  ...
  target=settingsReturnUrl(assertAllowedOrigin(saved.return_origin))
