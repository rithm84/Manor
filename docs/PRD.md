...
- **Calendar is a workspace**, not a sidebar tab: a top-bar toggle switches Workspace ↔ Calendar (Notion ↔ Notion Calendar model). The local-first Calendar is a real persisted workspace built on FullCalendar: day, week, and month views; mini-month navigation; calendar create/rename/recolor/show-hide; Today and previous/next navigation; search and a searchable command menu; first-day, weekend, working-hours, hour-density, primary/secondary-time-zone, and all-day controls; current-time and overlapping-event layout; and internal scrolling at the application minimum size. Timed events retain their declared IANA time zone, convert into the selected primary display zone, and reject wall times skipped by daylight saving transitions. Click or drag creates an event. Local events support centered create/edit, custom date and time controls, all-day and timed ranges, location, description, conferencing URL with a safe Open action, visibility, free/busy status, reminders, attached Manor Notes pages, duplicate/delete, drag-to-move, resize, and daily/weekly/monthly/yearly recurrence with intervals and date/count endings. Recurring changes explicitly choose This event or All events; occurrence exceptions and their parent-series identity persist, and deleting a series removes its exceptions. Local calendar deletion explicitly confirms that its events are removed. Home task due dates, scratch blocks, and Jobs milestones render as linked overlays without copying their source records. Scratch blocks remain Manor-only.
- **Calendar cloud boundary:** local calendar/event/settings state persists through typed SQLite/IPC. Google account connection, incremental Google sync, Notion database-calendar sync, invitations and RSVP delivery, scheduling-link publication, other people's availability, shared calendars, provider-created conferencing, and menu-bar integrations are not simulated. They remain integration work; the local UI reports Google as disconnected and accepts an existing conferencing URL only.
...
| FullCalendar 6 provides the official React adapter, time-grid/day-grid interaction plugins, efficient overlap layout, and an MIT license; its React peer range includes React 19 | Use FullCalendar for Calendar geometry and interaction instead of maintaining a custom time-grid engine |
