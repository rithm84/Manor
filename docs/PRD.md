...
Each day has one evolving synthesis updated through Codex. The page has no typed-context composer; typed debriefs happen in Codex, and existing manually written context stays readable with its original provenance. Quick capture accepts today and yesterday. In History, you can edit ratings for any past day or add a missing day in the saved account time zone; future dates are unavailable, and historical corrections preserve the daily synthesis and other existing fields. History matches the Habits history in structure: a month summary that names the average mood and focus in scale words, a 3, 6, or 12-month trend of monthly averages with rest days excluded from focus, a month calendar where each day names its mood and focus, marks whether a synthesis exists, and opens that day's editor with the synthesis and its provenance, and per-level distributions for the month. Debrief content is AI-accessible.
...
Notes is a cloud-backed document workspace with protected device-local drafts and queued saves. Pages keep folders, their hierarchy, favorites, recents, archive, Trash, full-text search, duplication, movement, Markdown import and export, and durable app-owned attachments. The Notion parity target concerns document editing; it doesn't extend Manor into Notion databases, shared workspaces, or general project management. Notes is two panes: the note list, whose header picker chooses the view (all notes, favorites, recent, archived, Trash, or a folder, with counts, and where folders are created and renamed), and the editor. The list collapses into a slim rail, preserving the open editor and remembering that choice on the device. Coming back to Notes during a session reopens the list and note you left, not the most recently edited one; a relaunch opens Notes with the list and no note selected. Moving between notes, starring, moving, trashing, and creating never wait for the cloud: the edit is protected on the Mac, the save continues in the background, and a note that fails to sync is offered back.
...
| 2026-09-23 | Markdown import with assets is an agent job: the `import_markdown_note` tool converts Markdown on the server and places uploaded files; the in-app import stays text only. Either way a document's opening heading becomes the note title (notes, agent tools). |
| 2026-09-23 | Notes drops the separate navigation pane: the list header's view picker holds the fixed views and folders, and the editor column widens (notes, surfaces). |
