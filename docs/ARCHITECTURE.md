...
![Sign-in and agent authorization: a person passes Google sign-in and the signup gate, whose grant the before-user-created hook requires; an agent client completes OAuth 2.1 discovery and the Manor consent screen to get a scoped token; both reach row-level security and the 113 operations.](diagrams/auth.svg)
