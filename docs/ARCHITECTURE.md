...
![Sign-in and agent authorization: a person passes Google sign-in and the signup gate, whose grant the before-user-created hook requires; an agent client completes OAuth 2.1 discovery and the Manor consent screen to get a scoped token; both reach row-level security and the 121 operations.](diagrams/auth.svg)
...
The [shared catalog](../supabase/functions/_shared/toolCatalog.ts) contains 121 executable operations (48 reads and 73 writes). Remote MCP exposes their individual names and restricts read-only grants to reads. Repeated constraints use standard JSON Schema references and shared constraints, and per-operation server validation is authoritative. There is no live-browser transport.
