...
- WebMCP uses the current Manor session. Remote MCP uses a separate scoped, revocable OAuth authorization for that same account; Google client credentials are not MCP credentials. Both adapters enforce the shared operation boundary. There is no independent agent superuser. Caller-provided provenance is context, not proof of authorization or a trustworthy actor identity.
...
Historical mood/focus rating corrections use a dedicated account-authenticated command with date and revision checks, preserving synthesis fields and recording ordinary action history. The command rejects OAuth agent clients and is not published in the WebMCP or remote MCP catalog; the History editor owns this exception to ordinary daily capture bounds.
...
Remote calls operate on committed data and return revisions and receipts through the same command boundary. They have no implicit access to open tabs, selections, unsynced drafts, or presentation controls. Publish only supported capabilities for each transport; browser-only calls return a specific unavailable-context error when no live context exists. Realtime/refetch updates open clients without overwriting drafts. WebMCP remains the live-page adapter; saved cloud-browser sign-in is a distinct mechanism and does not establish cloud WebMCP support.

### Current tool coverage and host connection

The [shared catalog](../supabase/functions/_shared/toolCatalog.ts) contains 112 executable operations (46 reads and 66 writes). Remote MCP exposes their individual names and restricts read-only grants to reads. [WebMCP](../app/src/web/WebMCP.tsx) groups those same exact contracts into 20 explicit domain read/write tools to fit browser host limits; each call selects a schema-validated operation and input without another discovery call or page navigation. [The browser grouping](../app/src/web/browserToolCatalog.ts) fails validation if an operation is missing or assigned twice. Nine additional browser tools expose context, selection, drafts, navigation, filters, block highlighting, and device-local appearance preferences. Repeated constraints use standard JSON Schema references and shared constraints. A registration guard and tests enforce the verified browser descriptor budget, including room for host-added metadata; per-operation server validation remains unchanged.
...
| Presentation and preferences | `open_object`, `reveal_blocks`, `show_filtered_view`, `get_preferences`, `update_preferences`, `get_integration_status` | UI presentation is explicit and preserves drafts, selection and focus. Safe appearance/view preferences are separate from timezone changes and credential/account management. No secrets returned. |
...
Return an explicit outcome (`committed`, `accepted`, `conflict`, or failure), affected IDs, committed versions and a bounded projection of changed data. Asynchronous responses additionally carry a run/upload ID and readiness state. Include a useful object link and separately reported browser presentation state where applicable. A committed result remains committed if the originating view disappears before rendering; do not report that the user saw a change merely because the server saved it.
