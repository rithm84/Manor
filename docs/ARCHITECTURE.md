...
Writes never change: every mutation still goes through `manor_command` with its idempotency identity and expected revision, and nothing is written locally first; pages keep their own optimistic state. The mirror holds full rows, including `revision`, for the tables the app reads (tasks, contexts, scratch blocks, saved views, habits and entries, mood and focus, LeetCode problems, attempts, and notes, job roles, transitions, and listings, bookmarks without their embeddings, resumes, weekly reviews, calendar accounts, calendars, and events, and the profile). Notes tables stay on their own path because the draft store and block merge already own that data, and files, series, and everything reached through functions or storage are not mirrored.
