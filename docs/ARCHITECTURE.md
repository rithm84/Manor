...
- Google-only signup gating and the OAuth token hooks are on with their persisted configuration checked; interactive Google sign-in was last completed against staging while the website existed, and the desktop leg is verified up to Google's page (see the Google client gap below).
...
- The local mirror bootstraps on first sign-in, applies an own command before the page refetches, picks up a change made through the Management API as another device within seconds, survives a cold relaunch with its cursor, and answers every mirrored route from disk: after the relaunch Home settled 22 ms after navigation and Habits, Jobs, Bookmarks, and LeetCode opened by deep link in 15 to 17 ms, against 185 to 742 ms from the server before.
- Notes reads from the mirror as well. On the staging bundle a note created and edited in the app reached the copy at its committed revision before the page refetched, an edit made through the Management API as another device showed in the open list within a second, opening a note re-read its last-opened time at an unchanged revision, an edit committed while the copy was still filling was applied by the same pass, and a cold relaunch onto Notes settled in 5 ms. Opening Notes from Home no longer starts two imports of the editor chunk, which the WebKit module loader before Safari 27 settles early for a graph with a top-level `await`; that raced the page as soon as its data came from disk.
- With `style-src` left as written, the staging bundle logged no policy refusals while the formatting toolbar remounted after a deleted selection, and every tooltip in the remounted toolbar stayed hidden, where the nonce build had shown 60 refused style attributes and the toolbar drawn unstyled (the desktop README's content security policy section has the detail). The toolbar measured 36 px around its 28 px buttons instead of 48, its color, block type, and link menus opened whole in both themes, the Notes status label showed its ring for the held beat before settling, and the habits overlay is covered by unit tests for a later click outranking an earlier result. On Home, deleting two tasks and pressing undo twice brought both back with the server at the restored revisions; before, a reload that overlapped a delete could hand the restore a stale revision.
...
`npm --prefix desktop run release` is the whole pipeline, and the installed app is the only thing that reads what it publishes. The following diagram shows both halves.

![Releases and updates: the release script builds the production identity, signs the archive with a minisign key kept outside the repository, writes latest.json for darwin-aarch64, and publishes the v-version release that the latest-release feed URL follows; the installed app checks that feed on launch, hourly, and on focus after an idle hour, offers a restart, waits while a dialog is open or an edit is unprotected, then installs an archive with a valid signature and relaunches.](diagrams/releases.svg)

The script refuses a working tree with uncommitted changes, because the published build would match no commit, and it refuses a version tag that already exists, because a published version is never replaced. A person installs a first copy by extracting the archive into Applications, and the app updates itself from then on. Details are in [desktop/README.md](../desktop/README.md).
...
![Sign-in and agent authorization: a person passes Google sign-in and the signup gate, whose grant the before-user-created hook requires; an agent client completes OAuth 2.1 discovery and the Manor consent screen to get a scoped token; both reach row-level security and the 122 operations.](diagrams/auth.svg)
...
An embedded webview cannot serve Google's authorization page, and an agent waiting in a browser has no way into the app except a link, so both flows leave for the system browser and come back on the build's URL scheme. The following diagram shows the three round trips and the single inbox that receives them.

![Leaving for the browser and coming back: Manor starts Google sign-in with skipBrowserRedirect and opens Supabase Auth and Google in the system browser, which returns manor://auth/callback with an authorization code; an agent's authorization request sends the browser to manor:/oauth/consent, because the OAuth server's site URL is the app scheme; a connection started in Settings returns manor://settings with a connection code from integration-callback. Every link becomes a route in one inbox, where the sign-in callback completes the session and every other route navigates.](diagrams/deep-links.svg)

Sign-in is the PKCE flow with the build's deep link as its return URL, and a verify link that returns session tokens in the fragment completes the same way; a refused sign-in shows its reason on the sign-in panel. The return URLs (`manor://auth/callback` in production, `manor-staging://auth/callback` in staging) are the only allowlisted ones, and the webview keeps the session as a browser would. The OAuth server accepts consent calls only from an origin on the redirect allowlist, so the webview origin `tauri://localhost` is listed there next to the sign-in return URL.
...
| Pomodoro | Sessions with kind, status, label, planned length, start, pause, end, focused seconds, and account-local date; a partial unique index keeps one running or paused session per account; timer preferences live in the profile's settings |
...
TanStack Query owns the app's server state, and query keys include the account identity and the filter or page parameters. Mirrored tables are answered from the local copy, where the app applies the filters and the ordering itself; everything else reads the server with server-side filtering, deterministic ordering, cursor pagination for growing collections, and bounded aggregate queries. A service that keeps rows for its revision checks merges each reload into what it already holds and keeps any row a receipt has moved past the read, so a read that overlapped a command cannot hand the next command a stale revision. A local change never reloads an entire module or invalidates the whole app.

The following diagram shows what a launch waits for, and what starts beside it rather than in front of it.

![What a launch waits for: the process starts the shell, which keeps the window hidden until the frontend has applied the saved theme and dispatched its first render, so the window appears in about 300 milliseconds; the session comes from local storage, a device that has opened the account before renders the cached account, the mirror opens ready, and Home reads it in about 20 milliseconds, while the profile read, the first pull, and the refresh of the feed-less tables run off the critical path.](diagrams/boot.svg)

A device that has opened the account before adopts the profile's name and time zone when they come back. Committed records returned by a command are applied to the cache immediately. Realtime `action_events` are one of the pokes that start a pull rather than a direct invalidation: the table is in the realtime publication, row-level security scopes its rows to their owner, the app ignores the echo of a command it issued itself, and the pull that follows invalidates only the tables it touched. Reconnect and window-focus reconciliation recover missed pokes, because Realtime is not a durable history. Stale versions arriving after newer ones are ignored, and drafts are protected separately from cached server records.
...
The desktop app reads from a copy of the account that the shell keeps in SQLite, so a page renders from the Mac and the network only refreshes it. The following diagram shows the parts and the direction each one talks in.

![Local mirror and sync: pages read through the gateway, which serves mirrored tables from the shell's SQLite mirror once it is ready and sends commands to manor_command as before; the mirror sync bootstraps every mirrored table, pulls the workspace change feed since its cursor, re-reads the changed rows from the tables, and writes upserts, deletes, and the new cursor into the mirror.](diagrams/mirror.svg)

Writes never change: every mutation still goes through `manor_command` with its idempotency identity and expected revision, and nothing is written locally first; pages keep their own optimistic state. The mirror holds full rows, including `revision`, for the tables the app reads (tasks, contexts, scratch blocks, saved views, habits and entries, mood and focus, pomodoro sessions, notes folders, pages, and suggestions, LeetCode problems, attempts, and mistakes, job roles, transitions, and listings, bookmarks without their embeddings, resumes, weekly reviews, calendar accounts, calendars, and events, and the profile). Notes pages carry their document content, so both the tree and an open note render from disk, while the draft store and the block merge keep owning unsaved typing. Note versions stay on the server: each one is a whole document kept for a superseded revision, and only version history and the conflict re-base read them. Note attachments, files, series, and everything reached through functions or storage are not mirrored.

The replication signal is the workspace change feed, not the action history: `manor_private.workspace_changes` records one row per changed row with a per-account contiguous cursor, the row's identity fields, the kind of change, and its revision, and it covers the worker-written calendar tables that the action history never sees. The action history omits revisions and skips updates that touch only excluded columns, so it stays an audit log. The app reads the feed through two owner-scoped functions, `manor_workspace_cursor` and `manor_workspace_changes`, which refuse agent tokens.

Sync is poke then pull: something tells the app that the account changed, and the app reads the feed past its own cursor. The following diagram shows one pull.

![One pull of the change feed: a poke from realtime, focus, coming back online, a one-minute timer, launch, or the app's own command starts a pull, which bootstraps when the mirror is empty or its cursor sits past the head; otherwise it reads pages of manor_workspace_changes, keeps the last change for each key, removes deleted and purged rows by key, skips a row whose local revision is already current, re-reads the rest by identity and upserts them or deletes the ones Postgres no longer returns, commits the page's cursor, invalidates the touched tables, and dispatches the committed event.](diagrams/pull.svg)

Pulling after the app's own command is what keeps the mirror ahead of the page's refetch without putting the socket on the write path. Two tables have no feed trigger: `job_listings`, a shared catalog that cron refreshes, and `leetcode_problems`, seeded once; both are replaced in full on launch and every hour. A change to `note_pages` is always re-read, whatever revision the copy already holds, because opening a note moves `last_opened_at` without moving its revision. Reading the head cursor before the bootstrap fills anything means a change committed while it runs lands after the recorded cursor and arrives with the next pull. Until the mirror is ready, reads use the server path.

Derived reads that Postgres computes, the habits state and the LeetCode summary, are cached in the mirror with the cursor and the account-local day they were computed for; a cached value is served only while both still match, so it is exact rather than stale. Signing out wipes the mirror, and a schema change rebuilds it. Reads work without a connection; saves still need one.
...
Remote calls operate on committed data and return revisions and receipts through the same command boundary. They have no access to the open window, selections, unsynced drafts, or presentation controls; Realtime and refetch update open clients without overwriting drafts.

### Current tool coverage and host connection

The [shared catalog](../supabase/functions/_shared/toolCatalog.ts) contains 122 executable operations (48 reads and 74 writes). Remote MCP exposes their individual names and restricts read-only grants to reads. Repeated constraints use standard JSON Schema references and shared constraints, and per-operation server validation is authoritative. There is no live-browser transport.
...
| Pomodoro | `query_pomodoro_sessions`, `start_pomodoro_session`, `pause_pomodoro_session`, `resume_pomodoro_session`, `end_pomodoro_session`, `log_pomodoro_session`, `update_pomodoro_session`, `delete_pomodoro_session` | Queries return finished sessions in a local date range with a focus summary for the range and the session in progress. Starting refuses while a session is in progress; ending records the focused time and accepts the completed outcome only once the planned length has run out; logging a past session takes an explicit start and duration. Timer preferences are account settings changed in the app. |
...
Markdown with local assets is imported through an agent rather than the app. The `import_markdown_note` operation converts Markdown on the MCP host (`markdownImport.ts`, built on `mdast`) into native blocks with fresh stable ids: headings, styled text and links, nested bullet, numbered, and task lists, quotes, code, tables, math, dividers, images, and file blocks. A revision of 0 creates the note through `import_note`, which checks attachment references; the current revision replaces an existing note's title and content through `update_note`. Local image and file paths become `manor-attachment://` URLs when the caller maps them and are otherwise returned in the receipt with the empty block awaiting each, so the agent can create the note, upload the files with the note as their parent, and import again with the mapping. The recipe an agent follows is the `manor-import` skill under `.agents/skills/`. Both paths title the note by an opening level-one heading and drop it from the body. The in-app import stays Markdown text only: remote images keep their address and local paths become empty image blocks.
