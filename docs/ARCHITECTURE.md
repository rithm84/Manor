...
- With `style-src` left as written, the staging bundle logged no policy refusals while the formatting toolbar remounted after a deleted selection, and every tooltip in the remounted toolbar stayed hidden, where the nonce build had shown 60 refused style attributes and the toolbar drawn unstyled (the desktop README's content security policy section has the detail). The toolbar measured 36 px around its 28 px buttons instead of 48, its color, block type, and link menus opened whole in both themes, the Notes status label showed its ring for the held beat before settling, and the habits overlay is covered by unit tests for a later click outranking an earlier result.
