...
> content, paper chrome, violet accent, Notion Calendar anatomy) — see
> `docs/DESIGN.md`. §B's data story remains canon; visual treatments in
> §A screens are historical.
...
- Jobs: To-apply new today(4): Anthropic — SWE Intern Agents (SF), Figma — Product Eng Intern (SF/NYC), Scale AI — SWE Intern ML Infra (SF), Ramp — SWE Intern (NYC); older: Modal (2d), Perplexity (3d). Action = "Mark applied". Pipeline (13 roles): Applied 7 (OpenAI Aug 19, Palantir Aug 18, Cohere Aug 17, +4), OA 2 (Databricks due Aug 22, Stripe due Aug 24), Interviews 3 (Vercel Interview 2 Aug 24, Notion Interview 1 unset, Linear Interview 1 Aug 21), Rejected: Meta (Aug 18). Board cards show only the current meaningful update. Flow begins at Applied and uses the later persisted stage transitions represented by this same story. Source: SimplifyJobs, parsed daily.
...
6. **MoodFocus** — a Daily↔History module surface. Daily uses two distinct one-tap selector rows (Mood 5, Focus 6), optional context, today/yesterday correction, and a debrief-with-Alfred path. History uses one shared six-month Mood/Focus chart, navigable month summaries, and a compact daily record with only today/yesterday editable. It is one continuous workflow, not a dashboard-card grid.
...
8. **Jobs** — to-apply list (4 new today amber-dotted + 2 older, "Mark applied" buttons, "applied &amp; seen auto-hide"); a Board/Flow toggle above the pipeline; board stage columns with counts, independent scrolling, and semantic current-update pills; responsive Sankey beginning at Applied with a hidden accessible value summary. Add Role and the complete editable role schema use centered dialogs with custom clearable date controls and Stage in the property layout. Source line: SimplifyJobs daily.
